...
Besides that, we want to mention that our novel method, GradePro, is not only very fast, but also yields high accuracy for diverse distributions. A detailed introduction of GradePro along with an extensive investigation of its accuracy is provided in [**cite?**]. 
